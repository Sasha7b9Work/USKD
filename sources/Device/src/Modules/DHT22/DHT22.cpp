...
// Прототипы функций
DataDHT DHT_getData(DHT_sensor *sensor); // Получить данные с датчика
...
DataDHT DHT::GetMeasuremets()
{
    if (HAL::IsLayout())
    {
        return DataDHT(22.0f, 66.0f);
    }
    ...
    return DHT_getData(&sensor);
}


DataDHT DHT_getData(DHT_sensor *sensor)
{
    DataDHT data(22.0f, 66.0f);
    ...
    {
        data.humidity = sensor->lastHum;
        data.temperature = sensor->lastTemp;
        return data;
    }
    ...
    while (getLine())
    {
        ...
        if (timeout > DHT_TIMEOUT)
        {
            ...
            sensor->lastHum = 66.0f;
            sensor->lastTemp = 22.0f;
...
#if DHT_POLLING_CONTROL == 1
    sensor->lastHum = data.humidity;
    sensor->lastTemp = data.temperature;
#endif
